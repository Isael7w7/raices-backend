...
| `GEMINI_PROJECT_ID` | Proyecto GCP para Gemini vía SDK `@google/genai` (fallbacks: `VERTEX_AI_PROJECT_ID` → `FIREBASE_PROJECT_ID`) | Consola GCP → Gemini Enterprise Agent Platform |
| `GEMINI_LOCATION` | Región (default: `us-central1`) | Consola GCP → Gemini Enterprise Agent Platform |
| `GEMINI_MODEL` | Modelo Gemini (default: `gemini-3.1-flash-lite`) | https://ai.google.dev/gemini-api/docs/models/gemini-3.1-flash-lite |
...
> (`vertexai: true`) sobre **Gemini Enterprise Agent Platform** (antes Vertex AI).
> Variables nuevas `GEMINI_*`; las `VERTEX_AI_*` legacy siguen aceptadas como fallback.
> Modelo por defecto: `gemini-3.1-flash-lite`. No se requiere API key: el SDK se autentica con ADC
